...
1. 爬虫引擎：通过包名，地区，语言自动构建 URL，抓取应用在 Google Play 和 App Store 的落地页内容。

2. 素材提取：精准识别并区分应用图标（Icon）与屏幕截图（Screenshots）。
3. 多维度查询：支持单地区即时查询和全地区查询对比。
4. 持久化存储：建立任务与图片的关联型数据库。

### 1.2 性能与可靠性需求

1. 缓存机制：设置 7 天数据有效期，减少重复爬取，降低封禁风险，提高稳定性。
2. 实时性：全地区对比采用流式数据传输，实现“即抓即显”的瀑布流效果。
3. 高并发：利用多线程技术处理全地区同步抓取，提升响应速度。
